...
public class Application01 {
    public static void main(String[] args) {
        ...
        * */
        ...
        // animal.jump();
        // animal2.bite();
        // 같은 주소값을 참조하고 있지만 부모의 자료형이라 찾지 못한다.
        ...
        // ((Tiger)animal1).bite();
        // ((Rabbit)animal2).jump();
        // 컴파일에서는 오류가 나지 않는데 런타임에서는 오류가 남


        //instanceof 연산자 확인
        System.out.println("--------instanceof 확인");
        System.out.println("animal1이 Tiger 와 같은지 "+ (animal1 instanceof Tiger));
        System.out.println("animal1이 Rabbit 와 같은지 "+ (animal1 instanceof Rabbit));
        System.out.println("animal1이 Animal 과 같은지 "+ (animal1 instanceof Animal));
        System.out.println("animal2가 Rabbit 와 같은지 "+ (animal2 instanceof Rabbit));
        System.out.println("animal2가 Tiger 와 같은지 "+ (animal2 instanceof Tiger));
        System.out.println("animal2가 Animal 와 같은지 "+ (animal2 instanceof Animal));

        if(animal1 instanceof Rabbit){
            ((Rabbit)animal1).jump();
        }

        Animal animal3 = new Rabbit();//자식을 부모에게 넣어주는 것 자동 형변환
        Animal animal4 = (Animal) new Rabbit();

       // Rabbit rabbit1 = (Rabbit) new Animal();
       Rabbit rabbit1 = (Rabbit) animal1;//다운캐스팅 명시적으로 꼭 써야한다
